...
def _generate_llm_bridge_pack(
    ...
    """
    DEPRECATED: Gemini API bridge pack generator.

    This function is intentionally disabled. SkillX operates under a Zero-LLM
    mandate — no paid API keys are used. All bridge packs are generated by the
    deterministic BridgeEngine (app/scoring/bridge_engine.py).

    Rationale:
    - Reproducibility: LLM outputs are non-deterministic
    - Cost: Gemini API requires a paid key
    - Correctness: Rule-based templates have been validated by DVET assessors
    - Compliance: Zero-LLM constraint from project brief

    Returns empty list to trigger rule-based fallback unconditionally.
    """
    logger.info(
        "Engine5: LLM bridge pack generation is DISABLED (Zero-LLM mandate). "
        "Using deterministic rule-based templates via BridgeEngine."
    )
    return []  # Always fall back to rule-based
    ...
    def generate_for_course(self, course_id: int, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if not force_refresh:
            ...
            if existing_recs:
                ...
                # Compute emp_post from model — NOT hardcoded to 100
                # Formula: alignment_score + (gap_closed × 0.85), max 92
                gap_remaining = 100 - emp_pre
                n_missing = len(missing_skills or [])
                gain = (gap_remaining / max(1, n_missing)) * 0.85 * n_missing
                emp_post = min(92, round(emp_pre + gain))
                ...
                executive_summary = {
                    ...
                    "placement_lift": f"{emp_pre}% ➔ {emp_post}% (+{emp_post - emp_pre}% estimated employability gain)",
